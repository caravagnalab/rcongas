...
mix_prior <-  list(NULL, torch$tensor(c(0.7,0.3)), torch$tensor(c(0.4,0.3,0.3)), torch$tensor(c(0.3,0.3,0.2,0.2)) )

subclones <- best_cluster(X = binned_express, clusters = 1:4, model = "MixtureGaussianNorm", steps = 400L, lr = 0.01, param_list = list(a = 0, b = 0.8, init_sd = 0.01), method = "AIC", mixture = mix_prior, posteriors = T) 
...
subclones2 <- best_cluster(final_expr[tumor_cells,], clusters = 1:4, model = "MixtureGaussianNorm", steps = 500L, lr = 0.01, param_list = list(a = 0, b = 1, init_sd = 0.01), method = "AIC") 
```
...
df_norm_tum <-  calculate_DE(GMB_smartseq_normal_vs_tumor, input = glioblastoma_TPM, clone1 = 3, NULL, normalize = F, logfc.threshold = 0.2)
  
df_5_no5 <- calculate_DE(GBM_smartseq_tumor_only, input = glioblastoma_TPM[,get_best_model(GBM_smartseq_tumor_only)$dim_names$cell_names], clone1 = 2, NULL, normalize = F, logfc.threshold = 0.2)
  
df_14_no14 <- calculate_DE(GBM_smartseq_tumor_only, input = glioblastoma_TPM[,get_best_model(GBM_smartseq_tumor_only)$dim_names$cell_names], clone1 = 1,3, normalize = F, logfc.threshold = 0.2)
