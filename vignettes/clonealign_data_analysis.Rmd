...
```{r}
set.seed(3)
res <- best_cluster(inp3,model = "MixtureGaussian", clusters = 1:4, steps = 400, lr = 0.05, param_list = list("theta_scale" = 300 / 100, "theta_rate" = 1/100, "cnv_var" = 0.60) , method = "BIC",  MAP = T, posteriors = F)

res_filt <-  filter_clusters(res, abundance = 0.1)
```
...
```{r}
library(Seurat)
de <- calculate_DE(res_filt, counts_SA[rownames(mat_pre),] %>%  t, clone1 = "1", clone2 = "2", method = "DESeq2")
```
