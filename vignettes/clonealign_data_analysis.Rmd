...
```{r}
set.seed(3)
res <- best_cluster(inp3,model = "MixtureGaussian", clusters = 1:4, steps = 800, lr = 0.01, param_list = list("theta_scale" = 3.2, "theta_rate" = 1, "cnv_var" = 0.50) , method = "BIC",  MAP = T, posteriors = F)
...
ggplot(clonealign_congas_merges %>%  filter(cluster != "C")) + geom_segment(aes(x = from, y = CNV, xend = to, yend = CNV, color = cluster)) +  facet_wrap(.~chr, scales = "free") + scale_color_manual(values = cols)
```
