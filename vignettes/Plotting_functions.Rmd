...
raw_input = get_input_raw_data(rcongas_example) %>%  t
...
random_30_genes = get_mapped_genes(rcongas_example) %>%
  dplyr::sample_n(30) %>%
  dplyr::pull(gene)
...
plot_raw_data(
  x = raw_input,
  genes = get_DE_table(rcongas_example, cut_pvalue = 1e-4, cut_lfc = 1) %>% dplyr::pull(gene), 
  clusters = get_clusters(rcongas_example), lognormalise = T,
  prompt = FALSE)
