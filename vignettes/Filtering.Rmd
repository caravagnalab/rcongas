...
Filtering functions are available for data to ba analysed before or after it is loaded into the package.

* **Before loading:** pre-process data that is ready for usage in the tool, returning an equivalent tibble with fewer rows;

* **After loading** process data that has already being inputed to the tool, returning a new \code{rcongasplus} object.

## Filtering count data

### Known genes

We have observed high variability across ribosomal (`r`) and mitochondrial (`m`) genes, which confounds signals in the data, 

We usually remove these genes, using function `filter_known_genes`. This function takes in input a `what` parameter (default `mr`) which
determines what genes should be removed, plus an extra `specials` list of Hugo Symbols to remove. 
...
# Remove only ribosomal 
example_input$rna %>%  filter_known_genes(what = 'r', specials = list())
```

### Outliers

Certain genes can have very high counts in certain cells. These outliers will affect the overall counts per segment used by `Rcongas` models, and can  be removed using function `filter_counts_by_quantile`, which computes an empirical quantile $q$ per event in the data (gene for RNA and peaks for ATAC).

The default quantile value for $q$ is 98% (`upper_quantile = .98`).

```{r}
# Filter
example_input$rna = example_input$rna %>% filter_counts_by_quantile
...
# A couple segments from the object
...
plot_data(example_object, what = 'histogram', segments = segment_id) %>% print

# Filter with 5%/95% values
example_object = example_object %>% filter_outliers(lower_quantile = .05, .95)
