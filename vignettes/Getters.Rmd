...
* `get_input_raw_data` and `get_counts`, `get_counts_matrix` to obtain the raw input data (i.e., counts) and the normalised counts.
...
The data processed by CONGAS are counts mapped to segments, which are stored in a matrix that contains cells on rows, and segments on columns. There are two types of getters for this information. The most simple one returns the actual data matrix.

```{r}
# Mapped input data - always stored inside the object
mapped_input_cell_by_segment = get_counts_matrix(rcongas_example)

# Just the 2x2 submatrix (cell on the rows, segments on the columns)
print(mapped_input_cell_by_segment[1:2, 1:2])
```

Once you have `mapped_input_cell_by_segment`, the getter `get_counts` can return all the data available for each cell, possibly normalized, or subset to certain portions of the cancer genome. This information also reports the cluster id for each of the input cells. 

```{r}
# Normalised data (default)
get_counts(rcongas_example, segments_input = mapped_input_cell_by_segment)

# A view on a certain chromosome 
get_counts(rcongas_example,
           segments_input = mapped_input_cell_by_segment,
           chromosomes = 'chr3')
