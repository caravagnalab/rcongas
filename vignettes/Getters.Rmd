...
* `get_input_raw_data` and `get_counts` to obtain the raw input data (i.e., counts) and the normalised counts.
...
# Just the 2x2 submatrix
print(raw_input[1:2, 1:2])
```

Once you have that data, the getter `get_counts` can return all the data available for each cell, possibly normalised, or subset to certain portions of the cancer genome.  

```{r}
# Normalised data (default)
get_counts(rcongas_example, input = raw_input)

# A view on a certain chromosome 
get_counts(rcongas_example, input = raw_input, chromosomes = 'chr3')

# Un-normalised data
get_counts(rcongas_example, input = raw_input, norm = FALSE)
```
