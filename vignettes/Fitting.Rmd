...
atac_counts = readRDS('lymphoma/tutorial/atac_counts.rds')
rna_counts = readRDS('lymphoma/tutorial/rna_counts.rds')
...
BiomaRt. A tutorial section on how to perform such conversion will be available soon

```{r}
features = readr::read_delim('lymphoma/filtered_feature_bc_matrix/features.tsv.gz', 
                                  delim  = '\t',
                                  col_names = c('gene_id', 'gene_name', 'modality', 'chr', 'from', 'to')) %>%
  filter(modality == 'Gene Expression') %>% dplyr::rename(gene = gene_name)
```
...
```{R}
x = Rcongas::filter_segments(x, RNA_nonzerocells = 6000, ATAC_nonzerocells = 6000)
```

Generate plots to check out the data distribution
```{R}
ggsave(
  paste0(fig.dir, "Histogram_longest_20_segments.png"),
  plot_data(
    x,
    what = 'histogram',
