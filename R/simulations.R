...
#' @param changes list of integers, indicating for each ploidy status the allowed changes (e.g., list('1' = c(1,2)) since we cannot go to 0 copies)
#'
...
change_spot <- function(parent_ploidy, mutation_spots, changes){
  for(i in mutation_spots){
    parent_ploidy[i] <- parent_ploidy[i] + sample(changes[[as.character(parent_ploidy[i])]], size = 1)
  }
  return(parent_ploidy)
}
