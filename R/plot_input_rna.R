...
#'
#' @examples
plot_counts_rna = function(x, input, normalised = TRUE)
{
  # RNA data
  RNA = get_counts(x, input, norm = TRUE) %>%
    idify() %>%
  ...
  rna_plot = ggplot(RNA) +
    ...
    labs(
      ...
      subtitle = paste0("RNA genes g = ", dim(M)[1], ", segments n = ", dim(M)[2], '.')
      ) +
