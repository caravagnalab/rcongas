...
plot_report <- function(cnv_sim,...) UseMethod("plot_report", cnv_sim)


plot_counts <- function(cnv_sim,...) UseMethod("plot_counts", cnv_sim)

plot_genome <- function(cnv_sim,...) UseMethod("plot_genome", cnv_sim)
...
plot.CNVSimulation <- function(cnv_sim, plot = T ){
  ...
  cell_cm_pl = plot_singlecell_gwide(
    N %>% mutate(n = n), legend = "ploidy"
  )
...
plot_report.CNVSimulation <- function(cnv_sim, file = "report.pdf"){
    ...
    align = 'vh',
    axis = "tblr",
    rel_heights = c(3,3, 1.5)) %>% ggsave(filename = file, width = 15, height = 12)
